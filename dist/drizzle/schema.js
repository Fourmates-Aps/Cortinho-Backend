import { pgTable, pgView, serial, smallserial, smallint, text, varchar, char, integer, numeric, boolean, date, timestamp, index, uniqueIndex, jsonb, } from "drizzle-orm/pg-core";
...
export const sealedPriceHistory = pgTable("sealed_price_history", {
    id: serial("id").primaryKey(),
...
// ─────────────────────────────────────────────────────────────
export const vCardPl = pgView("v_card_pl").as((qb) => qb
    .select({
    cardId: cards.id,
    userId: cards.userId,
    name: cards.name,
    ...
    plAbs: sql `ROUND(${cards.currentValue} - ${cards.purchasePrice}, 2)`.as("pl_abs"),
    plPct: sql `
...
export const vCollectionSummary = pgView("v_collection_summary").as((qb) => qb
    .select({
    userId: cards.userId,
    totalCards: sql `COUNT(*)`.as("total_cards"),
    collectionCount: sql `COUNT(*) FILTER (WHERE ${cards.status} = 'collection')`.as("collection_count"),
    wishlistCount: sql `COUNT(*) FILTER (WHERE ${cards.status} = 'wishlist')`.as("wishlist_count"),
    forSaleCount: sql `COUNT(*) FILTER (WHERE ${cards.status} = 'for_sale')`.as("for_sale_count"),
    gradedCount: sql `COUNT(*) FILTER (WHERE ${cards.isGraded} = TRUE)`.as("graded_count"),
    rookieCount: sql `COUNT(*) FILTER (WHERE ${cards.isRookie} = TRUE)`.as("rookie_count"),
    totalValue: sql `ROUND(SUM(${cards.currentValue}) FILTER (WHERE ${cards.status} = 'collection'), 2)`.as("total_value"),
    totalCost: sql `ROUND(SUM(${cards.purchasePrice}) FILTER (WHERE ${cards.status} = 'collection'), 2)`.as("total_cost"),
    totalPl: sql `ROUND(SUM(${cards.currentValue} - ${cards.purchasePrice}) FILTER (WHERE ${cards.status} = 'collection' AND ${cards.purchasePrice} IS NOT NULL AND ${cards.currentValue} IS NOT NULL), 2)`.as("total_pl"),
})
    .from(cards)
    .where(sql `${cards.deletedAt} IS NULL`)
    .groupBy(cards.userId));
export const vTransactionSummary = pgView("v_transaction_summary").as((qb) => qb
    .select({
    userId: transactions.userId,
    totalTransactions: sql `COUNT(*)`.as("total_transactions"),
    totalSpent: sql `ROUND(SUM(${transactions.price}) FILTER (WHERE ${transactions.type} = 'buy'), 2)`.as("total_spent"),
    totalEarned: sql `ROUND(SUM(${transactions.price}) FILTER (WHERE ${transactions.type} = 'sell'), 2)`.as("total_earned"),
    netPl: sql `ROUND(COALESCE(SUM(${transactions.price}) FILTER (WHERE ${transactions.type} = 'sell'), 0) - COALESCE(SUM(${transactions.price}) FILTER (WHERE ${transactions.type} = 'buy'), 0), 2)`.as("net_pl"),
})
    .from(transactions)
